# Create Virtulal Environment: python3 -m venv venv
# Acticate Virtual Environment: source venv/bin/activate
# Install Requirements: pip install -r requirements.txt
# Install env: pip install python-dotenv
# select right kernel
# RUN: Python3 1_01_JSON_Goal_to_PlanStructure.py

# Ingest: input_goal.json
# ALT WORKFLOW: FIRST: chmod +x run_all.sh   THEN:  ./run_all.sh 
...
"""
    ...
    try:
        ...
        # If the API gave a list for Evaluation_Criteria, convert it to a dict
        if isinstance(plan["Evaluation_Criteria"], list):
            plan["Evaluation_Criteria"] = {
                item["name"]: item["criteria"] for item in plan["Evaluation_Criteria"]
            }
...
def save_plan_structure(plan: Dict, base_filename: str = "plan_structure") -> bool:
    ...
    try:
        ...
        # Save both versions (one with timestamp, one without)
...
def main():
    """
    Main execution function that reads the goal from a JSON file
    and manages the plan structure generation process.
    """
    with tee_output():
        print("\nWelcome to the Project Plan Structure Generator!")
        print("This module will read your project goal from a JSON file and create the initial plan structure.")
        flush()

        # The default JSON file name is 'project_goal.json' if not passed in via command line
        json_filename = sys.argv[1] if len(sys.argv) > 1 else "project_goal.json"

        # Load the goal from the JSON file
        try:
            ...
            print(f"Error: The file '{json_filename}' is not valid JSON.")
            return

        if not project_goal:
            print("Error: Project goal cannot be empty or missing from the JSON file.")
            return
